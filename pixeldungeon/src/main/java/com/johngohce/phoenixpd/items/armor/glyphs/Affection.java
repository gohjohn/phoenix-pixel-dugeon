...
public class Affection extends Glyph {
	...
	public int proc( Armor armor, Char attacker, Char defender, int damage) {
		...
		if (Level.adjacent( attacker.pos, defender.pos ) && Random.Int( level / 2 + 5 ) >= 4) {
			...
			Buff.affect( attacker, Charm.class, Charm.duration(attacker) * duration ).object = defender.id();
			attacker.sprite.centerEmitter().start( Speck.factory( Speck.HEART ), 0.2f, 5 );
			
			duration *= Random.Float( 0.5f, 1 );
			
			Buff.affect( defender, Charm.class, Charm.duration(defender) * duration ).object = attacker.id();
